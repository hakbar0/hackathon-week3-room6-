...
| **What AI Generated** | `src/utils/eligibility.js` — a pure `checkEligibility(formData)` returning `{ outcome: 'eligible' \| 'partial' \| 'not-eligible', reasons, measures }` with hard gates (England, owner-occupier, EPC band D–G) and a soft income factor (≤ £36,000 → full, above → partial). `src/utils/eligibility.test.js` with 7 vitest cases. `CheckAnswersPage.jsx` — a `govuk-summary-list` driven by a row config, value→label maps, a Change `<Link>` per row, and a submit to `/result`. `ResultPage.jsx` — reads `formData`, calls `checkEligibility`, renders the green `govuk-panel--confirmation` for eligible/partial and a plain heading for not-eligible. Wired `formData` into the `/check-answers` and `/result` routes in `App.jsx`. |
| **What You Changed + Why** | (1) Dropped a custom `govuk-panel--not-eligible` grey-panel class I first reached for — it isn't in `govuk-frontend` (the old `App.css` was removed) and GOV.UK does not panel negative outcomes anyway; a plain `govuk-heading-xl` needs no custom CSS and matches the real pattern. (2) Guarded every hard gate on a *truthy* answer so an unanswered question (the skeleton pages don't store values yet) doesn't return a premature not-eligible — added a test for the empty-formData case. (3) Kept the eligibility decision entirely in `utils/` and out of the pages (CLAUDE.md §2): both pages only read state and render. (4) Used `<Link>` for Back and every Change action rather than `<a href>` so client-side navigation preserves the in-memory `formData`. |
